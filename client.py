...
class DataPublisher:
    ...
    async def turbine_control(self):
        ...
        if next_forecast_value is not "NA":      
            url_forecast = f"https://fra1.blynk.cloud/external/api/batch/update?token=RDng9bL06n9TotZY9sNvssAYxIoFPik8&v9={next_forecast_value}" #V9 Neykovo
...
async def main():
    ...
    opcua_client = OPCUAClient(        
        url = url_power, #url_power     
        client_app_uri="urn:freeopcua:client",
        cert_path=cert_base / "my_cert.pem",
        private_key_path=cert_base / "my_private_key.pem",
        wind_node = wind_node_neykovo,#wind_node_neykovo
        power_node = power_node_neykovo,#power_node_neykovo
        status_node = status_node_neykovo,#status_node_neykovo
    )
